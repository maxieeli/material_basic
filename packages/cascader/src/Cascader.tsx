...
const Cascader = forwardRef<CascaderRef, InternalCascaderProps>((props, ref) => {
  const {
    // MISC
    id,
    prefixCls = 'basic-cascader',
    fieldNames,
  ...
  const getValueByKeyPath = useCallback(
    (pathKeys: Key[]): SingleValueType[] => {
      const keyPathEntities = getPathKeyEntities()

      return pathKeys.map(pathKey => {
        const { nodes } = keyPathEntities[pathKey]

        return nodes.map((node: any) => node[mergedFieldNames.value])
      })
    },
  ...
  const [checkedValues, halfCheckedValues, missingCheckedValues] = useMemo(() => {
  ...
  props: PropsWithChildren<CascaderProps<OptionType>> & {
