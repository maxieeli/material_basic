...
class AjaxUploader extends Component<UploadProps> {
  ...
  onChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const acceptedFiles = [...files].filter(
      // @ts-ignore
      (file: BasicFile) => !directory || attrAccept(file, accept),
    )
  ...
  onFileDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (this.props.directory) {
      traverseFileTree(
        ...
        this.uploadFiles,
        (_file: BasicFile) => attrAccept(_file, this.props.accept),
      )
    } else {
      // @ts-ignore
      let files = [...e.dataTransfer.files].filter((file: BasicFile) => attrAccept(file, this.props.accept))
  ...
  uploadFiles = (files: File[]) => {
    const originFiles = [...files] as BasicFile[]
    const postFiles = originFiles.map((file: BasicFile & { uid?: string }) => {
   ...
   */
  processFile = async (file: BasicFile, fileList: BasicFile[]): Promise<ParsedFileInfo> => {
    ...
    const mergedParsedFile: BasicFile = parsedFile as BasicFile
