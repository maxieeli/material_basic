import type { BasicFile } from './interface'

interface InternalDataTransferItem extends DataTransferItem {
  isFile: boolean
  file: (cd: (file: BasicFile & { webkitRelativePath?: string }) => void) => void
  createReader: () => any
  fullPath: string
  isDirectory: boolean
  name: string
...
const traverseFileTree = (files: InternalDataTransferItem[], callback: any, isAccepted: any) => {
